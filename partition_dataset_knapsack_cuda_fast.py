...
def get_best_select(data, val_proportion, stride, use_var, loss_tolerance):
    ...
    dp_loss = tc.zeros((n + 1, prop_quant + 1)).cuda()  # 记录loss
    ...
    for i in tqdm(range(1, n + 1)):  # 从编号为1取数据,保证dp数组中的存在第0个数据以便加减获取
        for p in range(prop_quant + 1):
            ...
            # 相加的最小值
            former = get_loss_tc(dp_w[:i], prop)
            min_idx = torch.argmin(former)
            u,v = unravel_index(min_idx,former.shape)
            former = former[u][v]
            # 替换的最小值
            ...
            start = i if i-3<0 else i-3
            for j in range(0, i+1):  # 当前物体编号i-1，之前的物体编号从0到i-2
                dp_seq_select = dp_seq[start:i+1,start:i+1].clone() # torch内存公用
                if j < i:
                    dp_seq_select[:, :, j] = False # 循环设置每一行为False(删除该数据),最后保留一个完整的不设置的
                
                dp_seq_select = dp_seq[None,i if i-3<0 else i-3:i+1].repeat((2,1,1,1))
                dp_seq_select = dp_seq_select.reshape(-1,n)
                dp_w_select = weight_cls[1:][None, :].repeat((dp_seq_select.shape[0],1,1))
                ...
                if dp_loss_selelct[min_index] < min_loss:
                    min_loss = dp_loss_selelct[min_index]
                    w_select = dp_w_select[min_index]
                    seq_select = dp_seq_select[min_index]
            latter = min_loss
            if former < latter:
                dp_loss[i][p] = dp_loss[i - 1][p]
                dp_w[i][p] = dp_w[i - 1][p]
                dp_seq[i][p] = dp_seq[i - 1][p]
            else:
                dp_loss[i][p] = round(latter.item(),6)
    ...
    assert abs(loss_.item() - selection["best_prop_loss_var"][1]) < 1e-3, "correct loss: {}, but: {}".format(selection["best_prop_loss_var"][1], loss_)
