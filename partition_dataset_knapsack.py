...
def get_best_select(data, val_proportion, stride, use_var, loss_tolerance):
    ...
    # 权重格式化
    weight_cls = np.zeros((n + 1, m))
    weight_cls[1:] = data

    # 创建一个二维数组来存储子问题的解 数量*依次递减的loss
    prop_quant = len([_ for _ in np.arange(0, val_proportion, stride)])
    dp_w = np.zeros((n + 1, prop_quant + 1, m))  # 比例为主要状态,m细致的存储了每一个cls的权重
    dp_seq = np.full((n + 1, prop_quant + 1, n), False, dtype=bool)
    # 填充数组，从底部开始
    # loss渐进式，保证每个物体都能选择最合适的值来靠近val，但是存在缺陷
    # 由于dp_w保存的是每一个目标的权重，动态的get_loss获得值的并不稳定
    # 因为权重是动态累计的，那个位置的权重与现在的权重累计的loss更小是说不准的
    #      val_prop  --->
    # o   0.27 0.26 0.24 0.23 ....
    # b 0  w    w    w    w
    # j 1  w    w    w    w
    # | 2  w    w    w    w
    # | 3  w    w    w    w
    ...
    for i in tqdm(range(1, n + 1)):  # 从编号为1取数据,保证dp数组中的存在第0个数据以便加减获取
        for p in range(prop_quant + 1):
            ...
            # 使用：相加或替换
            former = get_loss(dp_w[i - 1][p], prop)  # [i-1][l]肯定是目前该位置loss的最小，但是动态的loss获取不一定是由它产生的
            # 相加的最小值
            # min_idx = np.argmin(get_loss(weight_cls[i] + dp_w[:i], loss_pro))
            # 替换的最小值
            # i种替换情况+1中相加情况 , 前i个物体种寻找做小loss, loss划分比例, 具体选了那些
            dp_seq_select = np.repeat(dp_seq[None, :i], i + 1, axis=0)
            for j in range(0, i):  # 当前物体编号i-1，之前的物体编号从0到i-2
                dp_seq_select[j, :, :, j] = False  # 循环设置每一行为False(删除该数据),最后保留一个完整的不设置的
            rep = (dp_seq_select.shape[0], dp_seq_select.shape[1], dp_seq_select.shape[2], 1, 1)
            dp_w_select = np.tile(weight_cls[1:][None, None, None, :], rep)  # 选择权重
            dp_w_select[~dp_seq_select] = 0  # 将False的地方设置为0
            dp_w_select = dp_w_select.sum(-2)  # 累计权重
            loss_selelct = get_loss(weight_cls[i] + dp_w_select, prop)
            min_index = np.argmin(loss_selelct)
            a, r, c = np.unravel_index(min_index, loss_selelct.shape)
            latter = get_loss(weight_cls[i] + dp_w_select[a, r, c], prop)
            # dp_w[i][p] = max(dp_w[i - 1][p],weights[i] + dp_w[i - 1][local_max_idx])
            if former < latter:
                dp_w[i][p] = dp_w[i - 1][p]
                dp_seq[i][p] = dp_seq[i - 1][p]
            else:
                dp_w[i][p] = weight_cls[i] + dp_w_select[a, r, c]
                dp_seq[i][p] = dp_seq_select[a, r, c]
                dp_seq[i][p][i - 1] = True  # 物体序号从0开始
    dp_loss = get_loss(dp_w, val_proportion)
    ...
    if use_var:  # 是否考虑方差
        if loss_tolerance is not None:  # 是否现选出一部分小的loss，再同时优化损失和方差
            ...
            # 在原始数据中找到对应的索引
            ori_indices_r, ori_indeces_c = np.where(condidate_mask)  # 原始行索引和列索引
            ...
            index = (loss_normal + vars_normal).argmin()
            r, c = index // (prop + 1), index % (prop + 1)
    else:  # 不考虑方差
        index = dp_loss.argmin()
        r, c = index // (prop + 1), index % (prop + 1)

    # 考虑或不考虑方差（使用）
    prop_best = np.nanmean(dp_w[r][c])  # 考虑方差和损失的最佳比例
    ...
    index = dp_loss.argmin()
    r_, c_ = index // (prop + 1), index % (prop + 1)
    prop_lossmin = np.nanmean(dp_w[r_][c_])
    # assert loss_min == loss[r_][c_]
    ...
    selection["row"] = r
    selection["column"] = c
    selection["best_prop_loss_var"] = (prop_best, loss_best, var_best)  # 最佳loss及其方差
    selection["lossmin_prop_loss_var"] = (prop_lossmin, loss_min, var_lossmin)  # 最小loss及其方差
    selection["dp"] = dp_w
    selection["weight_cls"] = weight_cls

    if change:  # 改变了
        selection["selected"] = ~seq_best
        selection["unselected"] = seq_best
    else:  # 没改变
        selection["selected"] = seq_best
        selection["unselected"] = ~seq_best

    # 验证序列的正确性
    w = np.sum(data[seq_best], axis=0)
    loss_ = get_loss(w, val_proportion)
    assert loss_ == selection["best_prop_loss_var"][1], "correct loss: {}, but: {}".format(selection["best_prop_loss_var"][1], loss_)
...
def partition_dp(df, val_proportion, stride=0.005, specific=True, use_var=False, loss_tolerance=None):
    ...
    # 验证loss
    w = np.sum(df.values[selected], axis=0)
    loss_ = get_loss(w, val_proportion)
    assert loss_ == selection["best_prop_loss_var"][1], "correct loss: {}, but: {}".format(selection["best_prop_loss_var"][1], loss_)
...
if __name__ == '__main__':
    ...
    selected_paths_all_work, unselected_paths_all_work = multi_partition_dp(df_dict, val_proportion, stride=0.005, specific=True, use_var=True,
                                                                            loss_tolerance=1.2)
