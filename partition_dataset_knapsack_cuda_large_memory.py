...
def get_best_select(data, val_proportion, stride, use_var, loss_tolerance):
    ...
    for i in tqdm(range(1, n + 1)):  # 从编号为1取数据,保证dp数组中的存在第0个数据以便加减获取
        for p in range(prop_quant + 1):
            ...
            # dp_seq_select = tc.repeat(dp_seq[None, :i], i + 1, axis=0)
            dp_seq_select = dp_seq[None, :i].repeat((i + 1,1,1,1))
            ...
            dp_w_select = tc.tile(weight_cls[1:][None, None, None, :], rep)  # 选择权重
            ...
            a, r, c = unravel_index(min_index, dp_loss_selelct.shape)
            latter = get_loss_tc(weight_cls[i] + dp_w_select[a, r, c], prop)
